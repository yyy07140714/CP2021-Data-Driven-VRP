...
class PredictOptimizeVRP:
    def __init__(self, rl_agent, state_encoder, assignment_handler, reward_calculator, distance_mat, epochs=3, min_epsilon_threshold=0.05, device='cuda', 
    cache_dir='baseline_cache', csv_routes='baseline_routes.csv', csv_summary='baseline_summary.csv', csv_training='training.csv', **kwargs):
        ...
        """
        self.rl_agent = QLearningAgent(n_states=100, n_actions=74) 
        self.state_encoder = state_encoder
        self.assignment_handler = AssignmentHandler(distance_mat, self.rl_agent)
    ...
    def fit(self, trgt, weekday, stops_list, n_vehicleslist, distance_mat, active_days, demands, capacities, training_days, log=None):
        ...
        capacities = [torch.tensor(c, device=self.device).item() if isinstance(c, (int, float)) else c for c in capacities]


        epoch_total_distances = []  # 用於存儲每個 epoch 的總距離
        ...
        print("capacities length:", len(capacities))
        print("capacities[0]:", capacities[0] if len(capacities) > 0 else "Empty")
        print("capacities[1]:", capacities[1] if len(capacities) > 1 else "Empty")
        ...
        with open(self.csv_training, mode='w', newline='') as file:
            ...
            for ep in tqdm(range(self.epochs), desc="Epochs"):
                ...
                total_distance_this_epoch = 0  # 當前 epoch 的總距離
                ...
                for day_idx, d in enumerate(tqdm(training_days, desc=f"Epoch {ep + 1} Days (Training)", leave=False)):
                    ...
                    while not done:
                        ...
                        for vehicle_idx, vehicle in enumerate(vehicles):
                            ...
                            action = self.rl_agent.select_action(
                                ...
                                current_position=position,  # 確保是 position
                    ...
                    row = [ep + 1, d, ad_diffcount, sd_diffcount, fulfilled_demand, total_distance_this_day]
                ...
                # 記錄每個 epoch 的數據
                epoch_total_distances.append(total_distance_this_epoch)
                ...
                print(f"Epoch {ep + 1} completed. Total Distance: {total_distance_this_epoch:.2f}, Average AD: {average_ad_per_epoch[-1]:.2f}, Average SD: {average_sd_per_epoch[-1]:.2f}")
                ...
                plt.plot(range(1, len(epoch_total_distances) + 1), epoch_total_distances, marker='o', label='Total Distance per Epoch')
                ...
                average_ad_per_epoch = [
                    ad.item() if isinstance(ad, torch.Tensor) and ad.numel() == 1  # 單元素張量
                    else ad.sum().item() if isinstance(ad, torch.Tensor)  # 多元素張量，計算總和
                    else float(ad) if isinstance(ad, (int, float))  # 如果已是數值，直接使用
                    else 0  # 預設值，避免錯誤
                    for ad in average_ad_per_epoch]

                average_sd_per_epoch = [
                    sd.item() if isinstance(sd, torch.Tensor) and sd.numel() == 1  # 單元素張量
                    else sd.sum().item() if isinstance(sd, torch.Tensor)  # 多元素張量，計算總和
                    else float(sd) if isinstance(sd, (int, float))  # 如果已是數值，直接使用
                    else 0  # 預設值，避免錯誤
                    for sd in average_sd_per_epoch]

                # ad/sd vs epoch
                plt.figure(figsize=(10, 6))
                plt.plot(range(1, len(average_ad_per_epoch) + 1), average_ad_per_epoch, marker='s', linestyle='--', linewidth=2, label='Average AD')
                plt.plot(range(1, len(average_sd_per_epoch) + 1), average_sd_per_epoch, marker='^', linestyle='-.', linewidth=2, label='Average SD')
                plt.xlabel('Epoch', fontsize=14)
            ...
            for d in test_days:
                ...
                while not done:
                    ...
                    for vehicle_idx, vehicle in enumerate(vehicles):
                        ...
                        # 選擇動作
                        action = self.rl_agent.select_action(state, candidate_stops, n_vehicles, current_position=position, distance_mat=distance_mat)
                        all_actions.append((vehicle_idx, action, state))
...
class AssignmentHandler:
    def __init__(self, distance_mat, rl_agent):
    ...
    def assign_stops_to_vehicles(self, n_vehicles, candidate_stops, demands, capacities, trgt, day, reward_calculator, state_encoder, rl_agent):
        ...
        """
        vehicles = [[0] for _ in range(n_vehicles)]  # 從 DC 出發
        ...
        while candidate_stops:
            ...
            for vehicle_idx, vehicle in enumerate(vehicles):
                ...
                # 為當前車輛選擇動作（直接使用 rl_agent）
                action = rl_agent.select_action(state, candidate_stops, n_vehicles, current_position, self.distance_mat, capacities, vehicle_idx, current_load, demands, day)
                all_actions.append((vehicle_idx, action, state))

            # 執行分配和更新
            for vehicle_idx, action, state in all_actions:
                if action in candidate_stops:
                    candidate_stops.remove(action)  # 從候選站點中移除
                    vehicles[vehicle_idx].append(action)  # 分配給車輛
...
class RewardCalculator:
    ...
    def compute_reward(self, formatted_solution, demands, distance_mat, trgt, d, capacities, current_state=None):
        ...
        # 獎勵計算
        # reward = normalized_fulfill + 1/distance_penalty * 100 - ad_diffcount * 0.01 - sd_diffcount * 0.01
        # reward = 100+ normalized_fulfill*100 - distance_penalty - ad_diffcount*0.01 - sd_diffcount*0.01
        # reward = 100 + normalized_fulfill * 500 - 0.1 * distance_penalty - 0.001 * ad_diffcount - 0.001 * sd_diffcount
        reward = normalized_fulfill*500 - distance_penalty*0.1 - ad_diffcount*0.1 - sd_diffcount*0.1
        # reward = normalized_fulfill*0.001 + 1/distance_penalty * 10000 
